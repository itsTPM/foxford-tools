...
async function handleThemes() {
    let themesIndex;
    try {
        themesIndex = await fetch("../../themes.json").then((response) =>
            response.json()
        );
    } catch (e) {
    ...
    const themesData = await Promise.all(
        themesIndex.map((theme) =>
            fetch(`../../themes/${theme}`).then((response) => response.json())
        )
    );
    ...
    if (selectedTheme) {
        ...
        const theme = themesData.find(
            (theme) => theme.name.toLowerCase() === selectedTheme
        );
...
async function checkForUpdates(meta) {
    const latestCommitFull = await fetch(
        "https://api.github.com/repos/itsTPM/foxford-tools/git/refs/heads/dev"
    )
